...
Iterable<Element> collectAllElementsFrom(
  ...
  return CachingIterable<Element>(DepthFirstChildIterator(rootElement, skipOffstage)).toList();
}

class DepthFirstChildIterator implements Iterator<Element> {
  DepthFirstChildIterator(Element rootElement, this.skipOffstage)
    : stack = _reverseChildrenOf(rootElement, skipOffstage).toList();

  final bool skipOffstage;

  Element _current;

  final List<Element> stack;
  ...
  bool moveNext() {
    ...
    _current = stack.removeLast();
    // Stack children in reverse order to traverse first branch first
    stack.addAll(_reverseChildrenOf(_current, skipOffstage));
